...
<header>
  <div id="header" style="background-color:#645DFD; height:200px;">
    ...
    <div class="moi">
      <h4 class="ass" style="color: white; font-size:24px; margin-top:30px"><?php echo $data['prenom']; ?> <?php echo $data['nom']; ?></h4>
      <p class="role" style="color: white;font-size:small;"><?php echo $data['roles']; ?> </p>
...
<body id="body" style="background-color:#F5F5F5;">
  ...
  <table class="table w-75 container h-100" style="background-color:#FFFFFF;">
    ...
    <tbody>
      ...
      $id = $data['id'];
      $list = $bdd->prepare("SELECT * FROM user WHERE etat=0  AND id!=$id ORDER BY matricule desc LIMIT $first,$mapage");
      $list->execute();
